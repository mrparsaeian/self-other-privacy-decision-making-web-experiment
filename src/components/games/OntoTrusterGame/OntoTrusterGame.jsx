...
class OntoTrusterGameClass extends React.Component {
  ...
  renderResults = () => {
    return (
      <div>
        <p>
          شما در ابتدا `$this.endoment` دریافت کردید و تصمیم گرفتید تا
          `$this.sentAmount` به شرکت کننده B بفرستید.
        </p>
      </div>
    );
  };
  ...
  renderTrusterDescription() {
    ...
    return (
      <div style={{ textAlign: "right" }}>
        ...
        <p>
          شما به صورت تصادفی و ناشناس با یک شرکت کننده دیگر جفت شده‌اید. یکی از
          شما به صورت تصادفی به عنوان شرکت کننده A و دیگری به عنوان شرکت کننده B
          انتخاب می‌شوید. قبل از اینکه هرگونه تصمیمی بگیرید به شما گفته می‌شود
          که شرکت کننده A خواهید بود یا شرکت کننده B .
        </p>
        <p>
          برای شروع شرکت کننده A مقدار `${this.endoment}` دریافت می‌کند و شرکت کننده
          B هیچ‌چیز دریافت نمی کند. شرکت کننده A می‌تواند مقدار دلخواه یا همه `$
          {this.endoment}` خود را به شرکت کننده B بدهد. قبل از اینکه شرکت کننده B این
          مقدار را دریافت کند، عدد ۳ در آن ضرب می‌شود. زمانی که شرکت کننده B
          مقدار ۳ برابر شده را دریافت کند می‌تواند تصمیم بگیرد که مقداری از آنرا
          به شرکت کننده A بفرستد.
        </p>
